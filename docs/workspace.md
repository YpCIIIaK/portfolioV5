...
Подключиться можно двумя способами — хватит одного.

### Способ 1 — внутренний токен (проще)

1. https://www.notion.so/my-integrations → *New integration*, тип **Internal**.
   Включи нужные Capabilities (Read/Update/Insert content).
2. Скопируй **Internal Integration Token** (начинается на `ntn_`) → `NOTION_TOKEN` в `.env`.
3. Открой [docs/workspace-schema.sql](./workspace-schema.sql) заново (создаёт `ws_integrations`
   для хранения выбора базы задач; для самого токена Supabase не обязателен).
4. Вкладка **Notion** сразу показывает «по токену» — можно искать/читать/создавать.

### Способ 2 — публичная интеграция (OAuth, кнопка «Подключить»)

1. Тип интеграции **Public**, Redirect URI `https://твой-домен/api/notion/callback`
   (локально `http://localhost:3000/api/notion/callback`) — он должен точно совпадать
   с доменом, с которого заходишь.
2. **OAuth client id / secret** → `NOTION_CLIENT_ID`, `NOTION_CLIENT_SECRET`. Нужен Supabase
   (токен владельца хранится в `ws_integrations`).
3. Вкладка **Notion** → **Подключить Notion** → выбери пространство и страницы.

### Общее для обоих способов

- **Дай доступ к страницам:** открой нужную страницу/базу в Notion → **⋯** →
  **Connections** → добавь интеграцию. Без этого API её не видит — это модель
  доступа Notion, одинаковая для внутренней интеграции и OAuth. Отличие лишь в том,
  что при OAuth страницы можно выбрать один раз в пикере при подключении.
- Во вкладке **Задачи · базы** отметь базу как источник задач.
- Доступ ко всем `/api/notion/*` — только владелец (`requireOwner()`); токен живёт
  только на сервере.
