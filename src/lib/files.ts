...
const hrSearch: FileNode = {
  ...
  language: "Markdown",
  blocks: [
    { t: "h1", text: "Поисковая платформа по кандидатам (стажировка)" },
    { t: "p", text: "HR-tech: полнотекстовый поиск по базе кандидатов поверх OpenSearch. React + NestJS. Моя зона — frontend выдачи и фильтров плюс backend-логика поиска и подготовка данных в индексе." },
    { t: "h2", text: "Самое ценное, что сделал" },
    { t: "p", text: "Поле totalExperience в индексе было нулём у всех кандидатов — range-фильтр по опыту просто не работал. Написал backfill-скрипт (scroll + _bulk update), пересчитавший опыт для 227k документов из исходных строк дат." },
    { t: "h3", text: "Расчёт реального стажа: слияние пересекающихся интервалов" },
    {
      t: "code",
      lang: "typescript",
      collapsible: true,
  ...
  return merged.reduce((acc, p) => acc + monthsBetween(p.start, p.end), 0);
    ...
    },
    {
      t: "metrics",
      items: [
        { label: "Документов пересчитано", value: "227 000" },
        { label: "Стек", value: "React 18, NestJS, OpenSearch, Postgres" },
        { label: "Инфра", value: "Docker, GitHub Actions, ArgoCD" },
      ],
    },
    { t: "tech", items: ["React", "TypeScript", "Redux Toolkit", "NestJS", "OpenSearch", "TypeORM", "Docker", "ArgoCD"] },
  ],
};
